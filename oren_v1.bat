...
REM Define the path to the IPFS config and swarm key
set CONFIG_FILE=%USERPROFILE%\.ipfs\config
set SWARM_KEY_FILE=%USERPROFILE%\.ipfs\swarm.key

REM Check if IPFS is initialized
if not exist "%CONFIG_FILE%" (
    echo IPFS is not initialized. Please run 'ipfs init' first.
    exit /b 1
)


REM Use PowerShell to modify the JSON config file
REM Modify the IPFS config using PowerShell
powershell -NoProfile -Command ^
"try { ^
    $config = Get-Content -Raw -Path '%CONFIG_FILE%' | ConvertFrom-Json; ^
    $config.Bootstrap = @('/ip4/YOUR_NODE_1_IP/tcp/YOUR_NODE_1_PORT/ipfs/YOUR_NODE_1_PEER_ID', '/ip4/YOUR_NODE_2_IP/tcp/YOUR_NODE_2_PORT/ipfs/YOUR_NODE_2_PEER_ID'); ^
    $config.Addresses.Swarm = @('/ip4/0.0.0.0/tcp/4001', '/ip4/0.0.0.0/udp/4001/quic'); ^
    ...
    Write-Host 'Failed to update the config file:' -ForegroundColor Red; ^
    Write-Host $_.Exception.Message; ^
    exit 1; ^
}"

if %errorlevel% neq 0 (
    echo Failed to update the config file. Check for PowerShell errors.
    exit /b 1
)
